...
CyclicDataGenerator::CyclicDataGenerator(size_t offset)
  : pattern(DEFAULT_CYCLIC_PATTERN)
  , slice_len(strlen(DEFAULT_CYCLIC_PATTERN))
{
  this->current_slice_index = offset % slice_len;
}

CyclicDataGenerator::CyclicDataGenerator(char* pattern, size_t offset)
  : pattern(pattern)
  , slice_len(strlen(pattern))
{
  this->current_slice_index = offset % slice_len;
}
...
void
CyclicDataGenerator::get_next_n(char* dst, size_t n)
{
  char buffer[n];
...
void
CyclicDataGenerator::get_prev_n(char* dst, size_t n)
{
  char buffer[n];
