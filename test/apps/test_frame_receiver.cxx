...
static inline int check_packet(const struct rte_mbuf& packet, int expected_size, uint32_t expected_type) {
...
static inline int check_against_previous_stream(const detdataformats::DAQEthHeader* daq_header){
	...
	if (prev_stream_exists){
		TLOG() << "prev timestamp of stream: " << prev_timestamp_of_stream[stream_id];
		TLOG() << "current timestamp of stream: " << stream_ts;
		uint64_t ts_difference = stream_ts - prev_timestamp_of_stream[stream_id];
		if (ts_difference != 2048){
			TLOG() << "WARNING: wrong timestamp difference " << ts_difference <<" for stream "<< stream_id <<" different from expected (2048)";
			ret_val=1;
			}
