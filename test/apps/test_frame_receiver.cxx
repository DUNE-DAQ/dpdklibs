    ...
    auto stats = std::thread([&]() {
        ...
        while (true) {
            ...
            for (auto& [lcore_id, stats] : core_stats) {
                ...
                if ((summed_min_payload_size > stats.min_payload_size) and (stats.min_payload_size != 0)){
                    summed_min_payload_size           = stats.min_payload_size;
                }
                if ((summed_min_size_report_difference > stats.min_size_report_difference) and (stats.min_size_report_difference != 0)){
