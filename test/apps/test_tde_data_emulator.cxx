...
static constexpr int rate_12bits = 3054;  // in Hz = 3.054 in ms
static constexpr int rate_16bits = 2290; // in Hz = 2.290 in ms
...
int main(int argc, char *argv[])
{
  ...
  // initializing the RateLimiter and t
  limiter.init();
  for(uint64_t i = 0; i < 100; i++)
  {
    while(i < toptime)
    {
      tdeheader.timestamp = i;
      limiter.limit();
    }
    
    std::string str = std::to_string(i);
    std::string filename16 = "/eos/home-d/dvargas/Salidadune/filesTDE16bits/binary_16bits_" + str + ".txt";
    std::string filename12 = "/eos/home-d/dvargas/Salidadune/filesTDE12bits/binary_12bits_" + str + ".txt";

    fp_16 = fopen(filename16.c_str(), "wb");
    if (fp_16 == NULL) {
        cout << "Could not open "
             << "output file fp_16" << endl;
        return -1;
    }
